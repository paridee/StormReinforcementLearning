...
public class ThresholdSystemWithCheck implements Runnable {
	...
	private static final Logger LOG = LoggerFactory.getLogger(ThresholdSystemWithCheck.class);
	public double min;
	public double max;
	public ConfigurationExecutor ex				=	new ConfigurationExecutor();
	
	public ThresholdSystemWithCheck(double minUtil,double maxUtil){
		this.min	=	minUtil;
		this.max	=	maxUtil;
	}
	...
	public void run() {
		...
		LOG.info("running threshold system with low check, parameters low th "+min+" high th "+max);
